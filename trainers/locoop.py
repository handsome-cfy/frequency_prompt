...
from trainers.frequency_prompt import FrequencyPrompt,FeatureMap2FrequencyBands
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        # 对 image_features 进行了归一化处理，使得每个特征向量的 L2 范数（或欧几里得范数）为 1
        # [32,512]
        image_features = image_features / image_features.norm(dim=-1, keepdim=True)

        # My Adaption
        image_features = self.frequency_prompt(image_features)
...
class LoCoOp(TrainerX):
    ...
    def build_model(self):
        ...
        self.register_model("frequency_prompt", self.model.frequency_prompt, self.optim, self.sched)
